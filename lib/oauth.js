...
exports.init = function (app) {
  ...
  var everyAuth = require('crafity-everyauth')
    ...
    , oauth = { findOrCreateUser: function (callback) {
      findOrCreateUserCallback = callback;
    } };

  function createFindOrCreateUserHandler(providerName) {
    return function (session, accessToken, accessTokenSecret, data, req) {
      var currentOAuth = this, promise = currentOAuth.Promise();
      setTimeout(function () {
        try {
          data.accessToken = accessToken;
          data.accessTokenSecret = accessTokenSecret;
          findOrCreateUserCallback.call(currentOAuth, providerName, req, session, data, promise);
        } catch (err) {
          console.log("err", err.stack, err);
          promise.fail(err);
        }
      }, 0);
      currentOAuth.redirectPath((req.auth && req.auth.redirectAfterLoginPath()) || "/");
      return promise;
    };
  }
