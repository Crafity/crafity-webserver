...
var fs = require('crafity-filesystem')
	...
	, stylusFileFilter = require("./fileserver.stylus.js")
	, auth = require('./auth')
	, oauth = require('./oauth')
...
exports.createServer = function (config, callback) {
	...
	if (config) {
		configureResources(null, config, configureServer);
	} else {
		configuration.open(function (err, config) {
			configureResources(err, config, configureServer);
		});
	}
	...
	function configureResources(err, config, callback) {
		if (err) {
			return callback(err);
		} else if (config && config.webserver.resources) {
			return resources.configure(config.webserver.resources,
				synchronizer.register("resources", function (err, resources) {
					callback(err, config, resources);
				}));
		} else {
	...
	function configureServer(err, config, resources) {
